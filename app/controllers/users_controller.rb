...
class UsersController < ApplicationController
  ...
  # SHOULD THESE METHODS EVEN BE ALLOWED in our Bibapp
  # so adding next before actually testing when these methods would be called
  before_action :authenticate_user!
  ...
  # commenting out make_resourceful for this version, not sure how to use this controller at this point
  # so adding code from above
  def index
  ...
  def edit
    # this is not correct and don't want to edit this way anyway
    #@user = current_user
    redirect_back(fallback_location: default_home_path)
  end
  
  def show
    # would only show logged in user, not correct
    #@user = current_user
    redirect_back(fallback_location: default_home_path)
  end
  ...
  #    flash[:notice] = t('common.users.flash_create', :email => @user.email, :login => @user.login, :locale => @user.default_locale)
